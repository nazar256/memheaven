...
- Search Console is verified for `https://nazar256.github.io/memheaven/`. At `2026-08-13T11:23Z`, URL Inspection reported `URL is in Google` / `Page indexed`; Googlebot Smartphone last crawled at `2026-08-13 07:27:35`, crawling and indexing were allowed, and the user-declared canonical was the Pages URL. The submitted sitemap row still reports `Could not read` / `0` discovered pages and no matching sitemap was found during URL Inspection; this is a separate sitemap-association issue, not evidence that the page is unindexable. Performance remains 0 clicks, 0 impressions, 0% CTR, average position 0, with no query rows for the available 11 Aug data. The tracked campaign contract, exact query/checkpoint protocol, and sanitized ordered baseline are in [`docs/DISCOVERABILITY_CAMPAIGN.md`](docs/DISCOVERABILITY_CAMPAIGN.md) and [`docs/DISCOVERABILITY_BASELINE_2026-08-12.json`](docs/DISCOVERABILITY_BASELINE_2026-08-12.json); detailed private evidence remains under `.tmp/campaign/`.
- Official MCP Registry verification on 2026-08-13 found exact-version active/latest `io.github.nazar256/memheaven` v0.1.2, titled `MemHeaven — Self-hosted Remote MCP Memory for ChatGPT`, with `websiteUrl=https://nazar256.github.io/memheaven/`. The branded `memheaven` search now returns v0.1.2, while generic searches still omit the server (`memory`: 30, `ChatGPT`: 10, `self-hosted`: 1, `remote MCP`: 0); continue waiting for generic search propagation before evaluating that hypothesis. This was an existing-record metadata correction, not a new directory submission.
- The current unpublished landing experiment is draft PR [#19](https://github.com/nazar256/memheaven/pull/19), branch `codex/remote-mcp-memory-content`. After the adjacent-intent audit found that relevant pages lead with a direct ChatGPT connector setup sequence, the branch gained a concise “How to give ChatGPT persistent external memory” three-step section; follow the linked PR and branch for the current pushed tip rather than a stale handoff SHA. Local tests, typecheck, lint, build, and diff checks pass. Keep it unmerged and unpublished through the formal 7-day manifest at `2026-08-20T05:52:50Z` so all three scheduled manifests remain comparable to the live PR #8 content. If it is published before or after that point, record the exact deployment time and restart a complete 24-hour/72-hour/7-day schedule for PR #19.
...
4. Continue the discoverability campaign at the recorded 24-hour, 72-hour, and 7-day checkpoints; do not change the fixed query set or create duplicate catalog listings. Re-verify Official MCP Registry search propagation for v0.1.2 and keep PR #19 unpublished through the 7-day checkpoint; any earlier publication requires recording deployment time and restarting the complete schedule.
