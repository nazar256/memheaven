...
- Official MCP Registry verification on 2026-08-13 found exact-version active/latest `io.github.nazar256/memheaven` v0.1.2, titled `MemHeaven — Self-hosted Remote MCP Memory for ChatGPT`, with `websiteUrl=https://nazar256.github.io/memheaven/`. The search endpoint is still serving a stale pre-v0.1.2 result set; recheck propagation before evaluating generic registry search discoverability. This was an existing-record metadata correction, not a new directory submission.
