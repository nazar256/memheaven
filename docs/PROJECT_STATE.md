...
- Completed hosted-client compatibility and low-effort catalog tasks; MemHeaven is currently visible in the Official MCP Registry, mcpservers.org, mcp.so, and Glama. MCP Find is not confirmed in the current public search snapshot; mcp.so's published listing is reachable but its internal `memheaven` search is stale while known searches work. Upstream discovery queues include punkpeye/awesome-mcp-servers PR #11926 and TensorBlock/awesome-mcp-servers issue #1703.
