...
- `npm test` ✅ (45 tests, including scope/isolation hardening and bootstrap/secret helper coverage)
- `npm run typecheck` ✅
- `npm run lint` ✅
- `npm run build` ✅
...
2. Optionally bind a custom domain and run `npm run init -- --base-url <custom-origin>` to patch OAuth/MCP URLs before redeploying.
3. Verify ChatGPT Developer Mode manually against the final workers.dev or custom domain endpoint.
