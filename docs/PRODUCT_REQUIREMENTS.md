...
- Docs: MCP tools reference, MCP integration, Palace concept, Knowledge graph concept, AAAK dialect, hooks, agents docs

## Compatibility matrix

| Original tool | Implemented? | Cloudflare behavior | Notes / rationale |
| --- | --- | --- | --- |
| `mempalace_status` | Yes | Status + Memory Protocol + backend capabilities + quota snapshot | Primary onboarding tool |
...
| `mempalace_get_aaak_spec` | Yes | Cloud AAAK guidance text | Preserved as protocol aid |
| `mempalace_search` | Yes | Workers AI embedding + Vectorize + D1/R2 hydration | Preserved semantics, storage redesigned |
...
| `mempalace_diary_write` | Yes | R2 verbatim diary + D1 metadata | Preserved |
| `mempalace_diary_read` | Yes | Tenant-scoped diary read from D1/R2 | Preserved |
...
3. Preserve `mempalace_status` as Memory Protocol onboarding.
