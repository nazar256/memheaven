...
- The Official MCP Registry now has active latest version `0.1.1` for `io.github.nazar256/memheaven`, with `websiteUrl` set to the canonical Pages landing page. The previous `0.1.0` record remains active but is no longer latest. The official publisher validated and published the tracked metadata; this corrected an existing authoritative record without creating a duplicate listing.
...
4. Monitor existing upstream review queues, including TensorBlock PR #1704, and verify that the Official MCP Registry continues to expose version `0.1.1` as latest; do not create duplicate catalog records.
