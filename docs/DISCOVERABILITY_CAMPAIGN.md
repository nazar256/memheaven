...
4. Monitor existing upstream review queues. When the owner-authenticated publisher flow is available, publish a new uniquely versioned `server.json` for the existing Official MCP Registry record, then verify the refreshed record through the official API; do not create a duplicate catalog record.
