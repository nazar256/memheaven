...
- The Official MCP Registry has one active latest record for `io.github.nazar256/memheaven` (version `0.1.0`). Its live `websiteUrl` still points to the GitHub repository, while the tracked and deployed metadata points to the canonical Pages landing page. The official publisher validates the tracked `server.json`; a metadata refresh is therefore a concrete existing-record correction with higher expected value than another generic directory submission. No duplicate record or registry mutation was made during this check.
...
4. Monitor existing upstream review queues, including TensorBlock PR #1704. When the owner-authenticated publisher flow is available, publish a new uniquely versioned `server.json` for the existing Official MCP Registry record, then verify the refreshed record through the official API; do not create a duplicate catalog record.
